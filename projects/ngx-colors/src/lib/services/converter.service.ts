...
export class ConverterService {
    ...
    if (hsva) {
      switch (format) {
        case ColorFormats.HEX: {
          const rgba: Rgba = this.hsvaToRgba(hsva)
          rgba.denormalize()
          output = this.rgbaToHex(rgba, true)
          break
        }
        case ColorFormats.HSLA: {
          const hsla: Hsla = this.hsva2hsla(hsva)
          hsla.denormalize()
          output = hsla.toString()
          break
        }
        case ColorFormats.RGBA: {
          const rgba: Rgba = this.hsvaToRgba(hsva)
          output = rgba.toString()
          break
        }
        case ColorFormats.CMYK: {
          const cmyk: Cmyk = this.hsvaToCmyk(hsva)
  ...
  public stringToFormat(color: string, format: ColorFormats) {
    const hsva = this.stringToHsva(color, true)
  ...
  public rgbaToHex(rgba: Rgba, _allowHex8?: boolean): string {
  ...
  public stringToHsva(colorString = '', allowHex8 = true): Hsva | null {
  ...
  public getFormatByString(color: string): string {
    if (color) {
      ...
      const regexHex = /(#([\da-f]{3}(?:[\da-f]{3})?(?:[\da-f]{2})?))/
      const regexRGBA = /(rgba\((\d{1,3},\s?){3}(1|0?\.\d+)\)|rgb\(\d{1,3}(,\s?\d{1,3}){2}\))/
      const regexHSLA = /(hsla\((\d{1,3}%?,\s?){3}(1|0?\.\d+)\)|hsl\(\d{1,3}%?(,\s?\d{1,3}%?){2}\))/
      const regexCMYK = /(cmyk\(\d{1,3}(,\s?\d{1,3}){3}\))/
